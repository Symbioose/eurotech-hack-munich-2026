# HONESTY.md

> Mandatory disclosure for the hackathon. This file lives at the root of your repository. Judges cross-check it against your code and your technical video.
>
> **The deal:** disclosed shortcuts are **not** penalized — that is the entire point of this file. Hidden ones are. Undisclosed pre-built code is heavily penalized, each undisclosed mock carries a small penalty, and a faked demo is heavily penalized. Telling the truth here costs you nothing.

---

## 1. Team — who did what
Judges compare this against `git shortlog -sn`, so keep it honest.

| Member | GitHub handle | Main contributions |
|---|---|---|
| Emile Jouannet | @Symbioose | Project lead. Full pipeline orchestrator, all AI agents (context, component, DFMA, RFQ, scene, intent), MCP servers (compliance, hardware, sourcing, supplier), all JSON data files, marketplace, export, frontend architecture |
| Remi Brenaut | @remiku | 3D BuildGuard visualization (explode / x-ray / per-component risk coloring / damage tooltips / occlusion), simulation loop integration, UI polish (collapsible panel, All-Components toggle) |
| Amaury Delille | @amaurydelille | ML lead for the world model: synthetic degradation simulator, PyTorch model, CEM planner, FastAPI world-model backend, evaluation workflow, initial Next.js scaffold and Zustand store, simulation UI early iteration, agent orchestration prototype, context gate |
| Joanne Jabbour | @joannejab | Business and GTM strategy, Hong Kong/GBA market positioning, pitch narrative, customer/buyer framing, multi-agent system prototype, early 3D scene attempt |

---

## 2. What is fully working
Features that run end-to-end on the live app, with real data and real logic. Be specific: name the feature, what input it takes, what output it produces.

- **AI pipeline (OpenAI GPT-4.1-mini when configured)** — user types a plain-English deployment problem; the app runs a typed context → compliance → component → assembly → BOM → DfMA → supplier → scene pipeline and streams results via SSE. With `OPENAI_API_KEY`, context/component/RFQ/scene steps use structured JSON calls. Without a key or on runtime failure, the app uses deterministic parser/rule/catalog fallbacks so the demo remains runnable.
- **World model (PyTorch backend)** — a 2-layer GRU + self-attention network is trained on synthetic BuildGuard degradation trajectories generated by our simulator. The simulator uses hard-coded HK climate normals and degradation formulas inspired by public climate/battery/corrosion/seal references; raw NASA/HK datasets are not loaded directly in this repo. The backend includes a persisted `world_model.pt`, CEM planner, FastAPI endpoints, and `evaluate.py`. Latest evaluation against the synthetic simulator: component MAE ≈ 0.014, failure-probability MAE ≈ 0.022, autoregressive drift ≈ 3x over 100 steps.
- **3D node visualization** — Three.js / React Three Fiber renders the smart-city node with explode mode, x-ray, per-component risk colour ramp (green → red), damage tooltips, occlusion check, orbit snap on component select, and a pause/resume rotation control.
- **MCP servers** — local Node.js stdio MCP servers exist for compliance, hardware, supplier routing, scene generation, and source research. Core tools read checked-in JSON data. Tavily-backed tools only return candidate web evidence when `TAVILY_API_KEY` is set; they do not automatically promote findings into trusted catalog/compliance data.
- **Marketplace redirect with click logging** — every "Buy" click goes through `/api/go`, is logged to `data/_marketplace-clicks.jsonl`, tagged with UTM params, and 302-redirected to the real distributor search page (LCSC / Digi-Key / Octopart).
- **Export** — PDF readiness pack (jsPDF, full A4 layout), machine-readable JSON, and CSV download all work client-side.
- **Context gate** — the pipeline refuses to run if the input is not a hardware deployment problem; it uses GPT-4.1-mini when `OPENAI_API_KEY` is configured and a deterministic local gate otherwise.

---

## 3. What is mocked, stubbed, or hardcoded
Every shortcut. Examples: a login that accepts any password, a payment that always succeeds, an "AI" that is an if/else, a database that is an in-memory dictionary, fake JSON returned instead of a real API call.

**Undisclosed mocks carry a small penalty each. Anything you list here = free.**

| What is faked | Where (file:line or folder) | Why we mocked it | What the real version would do |
|---|---|---|---|
| Component catalog | `frontend/data/component-catalog.json` | No time to wire a live distributor API during the hack | Pull live stock and pricing from Octopart / Nexar / LCSC API |
| Component catalog source status | `frontend/data/component-catalog.json` | 123 components total: 48 `verified` registry-backed entries and 75 `seeded` concept-stage entries | Live distributor/catalog integration with source refresh, approval workflow and stock/pricing APIs |
| Compliance rules database | `frontend/data/compliance-rules.json` | HK regulatory APIs (EMSD, PCPD, OFCA) require registration and approval | Live scrape / webhook from official HK government portals, versioned with last-checked timestamp |
| Supplier graph / RFQ route | `frontend/data/supplier-graph.json` | Seeded supplier-route templates for demo-stage RFQ planning; not live RFQs, verified relationships, binding quotes, or guaranteed supplier availability | Verified supplier CRM with contacts, MOQ, lead time, quote status, and audit trail |
| Parts pricing | `frontend/lib/pipeline/sourcing.ts` (priceFactor multipliers) | Octopart/LCSC pricing APIs require a paid tier | Real-time price + stock check per MPN via Nexar or LCSC open API |
| Buy-link availability | `frontend/lib/pipeline/sourcing.ts` | Links go to real search-URLs on Digi-Key / LCSC / Octopart but stock is not verified at request time | Verify stock and price at click time via API before rendering the button |
| Affiliate / revenue program | `frontend/app/api/go/route.ts` | UTM tags are appended and clicks are logged, but no real affiliate program is enrolled | Enroll in Digi-Key / Mouser affiliate programs; replace the placeholder ref tag with a real token |
| Deterministic pipeline fallbacks | `frontend/lib/pipeline/*`, `frontend/app/api/pipeline/fallback/route.ts` | Keeps the demo runnable without OpenAI or when streaming/API calls fail | Same typed pipeline with configured OpenAI/MCP calls and clearer runtime status badges |
| Scene fallback in OpenAI scene path | `frontend/lib/pipeline/scene-agent.ts` | If LLM scene output misses selected components, catalog scene metadata fills the missing nodes | Route every final scene through required Scene MCP, including the OpenAI path |
| World-model demo compare endpoint | `backend/main.py` `/demo/compare` | Stable pre-baked simulator curves for live demo comparison | Use `/plan` or `/compare` with the learned model for all demo curves |
| World-model training data | `backend/training.py`, `backend/demo_dataset.npz` | Synthetic trajectories generated by our simulator. Public climate/battery/corrosion/seal references calibrate constants and assumptions, but raw NASA/HK datasets are not ingested as training files in this repo | Add raw dataset ingestion scripts, source URLs, hashes, preprocessing notebooks, and real deployed sensor telemetry |
| Assembly patterns | `frontend/data/assembly-patterns.json` | Hand-authored during the hack | Generated from real IPC assembly standard rules |
| Demo object fixture | `frontend/data/demo-object.json` | Pre-seeded canonical scenario so the demo loads instantly without waiting for an LLM call | Any live pipeline run saved to disk |
| HK climate statistics in world model | `backend/training.py` (HK_MONTHLY_CLIMATE constants) | Copied from published HK Observatory averages (1961–2020), not from a live API | Subscribe to HK Observatory open-data feed for live environmental conditions |

---

## 4. External APIs, services & data sources
Everything the project calls or pretends to call. Mark each as real or mocked.

| Service / API / dataset | Used for | Real call or mocked? | Auth (sandbox / test key / none) |
|---|---|---|---|
| OpenAI GPT-4.1-mini | Context agent, component agent, RFQ agent, scene agent, intent classifier, context gate | **Real calls when configured** — deterministic fallbacks when absent or failing | Own paid API key (`OPENAI_API_KEY`) |
| Tavily Search API | Live web evidence for compliance and hardware MCP tools | **Real calls** — falls back to static data gracefully when key absent | Own free-tier key (`TAVILY_API_KEY`) |
| Python FastAPI backend (`http://127.0.0.1:8000`) | World-model plan, compare, and stress-test endpoints | **Real calls** — Next.js auto-starts the process | Local process, no external auth |
| LCSC / Digi-Key / Octopart | Buy-link redirect (search-URL, not an API call) | **Search URL only** — no API call is made | None |
| HK Observatory climate averages | Seed distribution parameters for world-model synthetic training data | Used as hard-coded constants — no live API call | None |

---

## 5. Pre-existing code
Anything written **before** kickoff that we brought into this project: prior personal projects, forked open-source code, templates, boilerplate, internal libraries.

**Undisclosed pre-built code is heavily penalized. Anything you list here = free.**

The repository started empty for the hackathon. All project code, data files, docs, UI, backend code, model code and demo assets in this repo were created during the hackathon window.

No personal side-projects, private libraries, forked repositories, templates with prebuilt product logic, or internal codebases were brought in.

We installed normal open-source dependencies during the hackathon through `package.json` and `backend/pyproject.toml` (for example Next.js, React, Three.js, OpenAI SDK, MCP SDK, PyTorch and FastAPI). These are dependencies only, not pre-existing project code.

---

## 6. Known limitations & next steps
What we would build next, and the weak spots we already know about. Naming these honestly is a strength, not a flaw.

- **World model validation is simulator-only** — the backend/model/planner/evaluation are real, but the checkpoint is trained and evaluated against synthetic simulator trajectories, not deployed BuildGuard hardware logs; accuracy on real deployments is unknown.
- **Component catalog and compliance rules are static** — new components, price changes, EOL notices, and regulation updates require a manual file edit; a live Octopart/Nexar + HK government feed integration would fix this.
- **Single city (Hong Kong / GBA)** — deployment context is filtered to HK; extending to other smart-city contexts (Singapore, Dubai, Munich) requires adding compliance and climate datasets for those cities.
- **No user auth or server-side project persistence** — projects are stored in `localStorage`; a backend database would be needed for team sharing and history.
- **No real supplier verification** — the GBA supplier route is illustrative; real procurement would require verified contacts, NDA flows, and MOQ negotiation.
- **Scene MCP is not yet mandatory in every path** — deterministic generation uses required Scene MCP; the OpenAI scene path validates and fills scene nodes from catalog metadata instead of always calling Scene MCP.
- **Next step: broader component and object library** — expand from the current smart-city node catalog into a much larger library of components, object families, assembly patterns, DfMA rules, scene metadata and supplier mappings.
- **Next step: foundation-style physical world model** — train a broader pretrained model for physical-product behavior across more object types, locations and deployment contexts. We do not claim perfect simulation today; the goal is to move from one BuildGuard-specific learned stress layer toward a general physical world model that can guide what experts should validate first.
- **Next step: fine-tuned specialist agents** — replace some generic structured LLM calls with fine-tuned agents for context extraction, component selection, compliance, sourcing, RFQ generation and scene generation, using project outcomes and expert review as training data.
- **Next step: live MCP knowledge servers** — connect MCP servers to continuously updated truth sources: distributor APIs, datasheets, compliance portals, supplier CRMs, quote histories, certification requirements and field-test outcomes. Today those sources are mostly checked-in files; the production version needs live update pipelines and human approval workflows.
- **Why Hong Kong / GBA matters next** — these next steps require supplier relationships, real quote data, pilot operators, hardware experts, compliance partners and capital. Hong Kong is the trusted front door for those pilots and partnerships; the Greater Bay Area is the manufacturing network where the data and feedback loops can become real.
